...
class CBXPoll_Session
{
    ...
    public function __construct()
    {
        ...
        if ($this->use_php_sessions) {
            ...
            if (!defined('WP_SESSION_COOKIE')) {
                define('WP_SESSION_COOKIE', 'cbxpoll_wp_session');
            }

            if (!class_exists('Recursive_ArrayAccess')) {
                require_once CBX_POLL_PLUGIN_ROOT_PATH.'includes/libraries/class-recursive-arrayaccess.php';
            }

            if (!class_exists('WP_Session')) {
                require_once CBX_POLL_PLUGIN_ROOT_PATH.'includes/libraries/class-wp-session.php';
                require_once CBX_POLL_PLUGIN_ROOT_PATH.'includes/libraries/wp-session.php';
            }
    ...
    public function init()
    {
        ...
        if ($this->use_php_sessions) {
            $this->session = isset($_SESSION['cbxpoll'.$this->prefix]) && is_array($_SESSION['cbxpoll'.$this->prefix]) ? $_SESSION['cbxpoll'.$this->prefix] : array();
        } else {
    ...
    public function set($key, $value)
    {
        ...
        if ($this->use_php_sessions) {
            ...
            $_SESSION['cbxpoll'.$this->prefix] = $this->session;
        }
     ...
     * or if the CBXPOL_USE_PHP_SESSIONS constant is defined
     *
    ...
    public function use_php_sessions()
    {
        ...
        $cbxpoll_use_php_sessions = (bool) get_option('cbxpoll_use_php_sessions');

        if (!$cbxpoll_use_php_sessions) {

            // Attempt to detect if the server supports PHP sessions
            if (function_exists('session_start')) {

                $this->set('cbxpoll_use_php_sessions', 1);

                if ($this->get('cbxpoll_use_php_sessions')) {

                    $ret = true;

                    // Set the database option
                    update_option('cbxpoll_use_php_sessions', true);
            ...
            $ret = $cbxpoll_use_php_sessions;
        }

        // Enable or disable PHP Sessions based on the CBXPOL_USE_PHP_SESSIONS constant
        if (defined('CBXPOLL_USE_PHP_SESSIONS') && CBXPOLL_USE_PHP_SESSIONS) {
            $ret = true;
        } else {
            if (defined('CBXPOLL_USE_PHP_SESSIONS') && !CBXPOLL_USE_PHP_SESSIONS) {
                $ret = false;
        ...
        return (bool) apply_filters('cbxpoll_use_php_sessions', $ret);
    }

    /**
     * Determines if a user has set the CBXPOLL_USE_CART_COOKIE
     *
    ...
    /*public function use_cart_cookie() {
        ...
        if ( defined( 'CBXPOLL_USE_CART_COOKIE' ) && ! CBXPOLL_USE_PHP_SESSIONS ) {
            $ret = false;
        }

        return (bool) apply_filters( 'cbxpoll_use_cart_cookie', $ret );
    }*/
    ...
    public function should_start_session()
    {
        ...
        return apply_filters('cbxpoll_start_session', $start_session);
    ...
    public function get_blacklist()
    {
        ...
        $blacklist = apply_filters('cbxpoll_session_start_uri_blacklist', array(
            'feed',
            'feed/rss',
            'feed/rss2',
            'feed/rdf',
